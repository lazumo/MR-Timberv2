...
public class SliceObject : NetworkBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (Physics.Linecast(startPoint.position, endPoint.position, out RaycastHit hit, sliceLayer))
        {
            ...
            // --- YOUR ORIGINAL DEBUG INFO PRESERVED ---
            Debug.Log(
    ...
    void RequestSliceServerRpc(ulong objId, Vector3 point, Vector3 normal)
    {
        ...
        {
            // 1. Get the WoodTree Script
            WoodTree treeScript = netObj.GetComponent<WoodTree>();

            if (treeScript != null)
            {
                // 2. Spawn Loot (The new public function)
                treeScript.SpawnMaterials();

                // 3. Notify Spawner (So a new tree grows in 5s)
